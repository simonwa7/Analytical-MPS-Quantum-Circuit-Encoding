...
def run_and_plot(molecule):
    ...
    mps_error_history = [
        abs(energy - ground_state_energy) for energy in mps_cost_history
    ]
...
for chain_length in [2, 3, 4, 5, 6]:
    molecule = MolecularData(
        geometry=[["H", [0.0, 0.0, (i * 0.7414)]] for i in range(chain_length)],
        basis="sto-3g",
        multiplicity=1 + (chain_length % 2),
        charge=0,
    )
